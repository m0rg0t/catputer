...
Use Lofi Cat as a reference for calm pacing, a prominent cat and separate room/character layers. Create original room and cat masters with imagegen, then adapt the composition to a 240 × 135 pixel grid with deliberate pixel clusters and a readable silhouette. The current shared 64-color palette reserves 16 entries for the UI and 48 for the jointly quantized room and cat. The cat should occupy approximately 48–64 pixels in height; test the exact placement against the HUD. Preserve an explicit cat anchor/bounds in each scene asset instead of relying on automatic centering.
