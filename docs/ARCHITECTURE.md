...
Current format 4 uses 192 bytes and retains the eight favorite records, with tempo/meter/tone bytes, an auto-dim setting in byte 7, and a CRC. The bounded decoder also accepts valid 160-byte format-1/2 files and 192-byte format-3 files; migration defaults auto-dim to 60 seconds. Legacy favorite schemas remain unavailable for replay under the new composer. Sleep timer state is runtime-only. Persist settings/favorites with a temp file, flush/close, recoverable rename/backup sequence and boot recovery. FAT rename is not treated as a universal power-loss guarantee. Avoid writes on every key repeat. Never replace the whole user's `/LOFI` directory during updates.
