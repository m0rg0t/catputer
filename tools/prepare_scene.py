...
def render_include(data: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "",
        "static constexpr std::uint8_t kGeneratedSceneBackground[240 * 135] = {",
    ]
    lines.extend(_format_values(data["background_indices"]))
    lines.extend([
        "};",
        "",
        "static constexpr std::uint8_t kGeneratedSceneCat[6][64 * 72] = {",
    ])
    ...
    lines.extend([
        "};",
        "",
        "static constexpr std::uint16_t kGeneratedScenePalette[64] = {",
    ])
    lines.extend(_format_values(palette565, 12))
    lines.extend([
        "};",
        "",
        "static constexpr std::uint32_t kGeneratedScenePaletteRgb[64][3] = {",
    ])
    ...
    lines.extend([
        "};",
        "",
        "static constexpr std::uint8_t kGeneratedSceneTransparent = 255;",
        f"static constexpr int kGeneratedSceneOriginX = {CAT_ORIGIN_X};",
        f"static constexpr int kGeneratedSceneOriginY = {CAT_ORIGIN_Y};",
        "",
    ])
    return "\n".join(lines)
...
def write_outputs(data: dict[str, Any], include_path: Path, runtime_dir: Path, project_root: Path = ROOT) -> dict[str, Any]:
    """Write firmware include, indexed previews, and a provenance manifest."""

    pillow = _require_pillow()
    include_path.parent.mkdir(parents=True, exist_ok=True)
    runtime_dir.mkdir(parents=True, exist_ok=True)
    rendered_include = render_include(data)
...
def main() -> int:
    ...
    try:
        ...
        if not args.validate_only:
            write_outputs(data, args.output_include, args.runtime_dir, args.project_root)
        print(
